	...
	for (x = 0; x < zonesX; x++) {
		for (y = 0; y < zonesY; y++) {
			...
			if (newArr.Persistent.StrandedWorld.PlayerZoneId.localeCompare(currentZone.Id) == 0)
			{
				...
				//let calcy = y * zoneYwidth + 0.5 * zoneYwidth - Number(newArr.Persistent.StrandedWorld.PlayerPosition["z"]) * (height / 1000);
				...
				// debug
				//let calcx_test = x * zoneXwidth + 0.5 * zoneXwidth;
				//let calcy_test = y * zoneYwidth + 0.5 * zoneYwidth;

				//let imageObj_debug = new Image();
				//imageObj_debug.style.zindex = 7000;
				//imageObj_debug.src = './icons/player.png';
				//imageObj_debug.onload = function () {
				//	ctx.drawImage(imageObj_debug, calcx_test - 12, calcy_test - 12, 24, 24);
				//};
				// end debug
