...
class FastRandom {
	...
	Next(lowerBound, upperBound)
	{
		...
		if (this.isDebug) {
			...
			writeDeveloperLog(this.x+"$"+this.y+"$"+this.z+"$"+this.w+"$"+num+"$"+num2+"$"+result);
		}
		return result;
	}
	...
	NextDouble()
	{
		...
		this.w = ((this.w ^ (this.w >>> 19) >>> 0) ^ ((num ^ (num >>> 8)) >>> 0)) >>> 0;
		let result = (4.6566128730773926E-10 * Math.trunc(2147483647 & this.w)).toPrecision(17);
		if (this.isDebug) {
			//console.log(this.x+"$"+this.y+"$"+this.z+"$"+this.w+"$"+num+"$0$"+result);
			writeDeveloperLog(this.x+"$"+this.y+"$"+this.z+"$"+this.w+"$"+num+"$0$"+result);
		}
		return result;
	}
}