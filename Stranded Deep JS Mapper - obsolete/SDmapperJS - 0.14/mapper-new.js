// Stranded deep Mapper by Hantacore - v0.14 / 0.72 big world compatibility
var debug = false;
var dragging = false;
var lastX;
var lastY;
...
function drawMapElements(focusOnPlayer) {
	...
	for(zoneIndex = 0; zoneIndex < generationZonePositions.length; zoneIndex++)
	{
			...
			if (drawZoneNames) {
				ctx.fillStyle = '#ccffff';
				ctx.fillRect(x - zoneXwidth + 5, y - zoneYwidth + 6, 500, 20);
				ctx.fillStyle = 'black';
				ctx.font = "10px Arial";
			...
			{
				// calculated position
				//let calcx = x + (Number(String(newArr.Persistent.StrandedWorld.PlayersData[0].Position["x"]).replace(",","."))) * (width / 1000 / widthratio); // local position conversion from center of island
				//let calcy = y + (Number(String(newArr.Persistent.StrandedWorld.PlayersData[0].Position["z"]).replace(",","."))) * (height / 1000 / heightratio);  // local position conversion from center of island
				
				let calcx = (width/2) + (Number(String(newArr.Persistent.StrandedWorld.PlayersData[0].Position["x"]).replace(",","."))) * distancescalefactor;// * (width / 1000 / widthratio); // local position conversion from center of island
				let calcy = (height/2) - (Number(String(newArr.Persistent.StrandedWorld.PlayersData[0].Position["z"]).replace(",","."))) * distancescalefactor;// * (height / 1000 / heightratio);  // local position conversion from center of island
			...
			{
				// calculated position
				let calcx = x + (Number(String(newArr.Persistent.StrandedWorld.PlayersData[1].Position["x"]).replace(",","."))) * (width / 1000 / widthratio); // local position conversion from center of island
				let calcy = - y - (Number(String(newArr.Persistent.StrandedWorld.PlayersData[1].Position["z"]).replace(",","."))) * (height / 1000 / heightratio);  // local position conversion from center of island
