// Stranded deep Mapper by Hantacore - v0.11 / 0.51 compatibility
var debug = false;
var dragging = false;
var lastX;
var lastY;
...
function drawMapElements(focusOnPlayer) {
	...
	for (x = 0; x < zonesX; x++) {
		for (y = 0; y < zonesY; y++) {
			...
			if (newArr.Persistent.StrandedWorld.PlayersData.hasOwnProperty("[0]Player")
				&& newArr.Persistent.StrandedWorld.PlayersData["[0]Player"].ZoneId.localeCompare(currentZone.Id) == 0)
			{
				// calculated position
				let calcx = x * zoneXwidth + 0.5 * zoneXwidth + (Number(String(newArr.Persistent.StrandedWorld.PlayersData["[0]Player"].Position["x"]).replace(",",".")) - (x - 2) * 630) * (width / 1000); // local position conversion from center of map, zone is 630 wide in game units
				let calcy = y * zoneYwidth + 0.5 * zoneYwidth - (Number(String(newArr.Persistent.StrandedWorld.PlayersData["[0]Player"].Position["z"]).replace(",",".")) + (y - 2) * 630) * (height / 1000);  // local position conversion from center of map, zone is 630 high in game units
			...
			if (newArr.Persistent.StrandedWorld.PlayersData.hasOwnProperty("[1]Player")
				&& newArr.Persistent.StrandedWorld.PlayersData["[1]Player"].ZoneId.localeCompare(currentZone.Id) == 0)
			{
				// calculated position
				let calcx = x * zoneXwidth + 0.5 * zoneXwidth + (Number(String(newArr.Persistent.StrandedWorld.PlayersData["[1]Player"].Position["x"]).replace(",",".")) - (x - 2) * 630) * (width / 1000); // local position conversion from center of map, zone is 630 wide in game units
				let calcy = y * zoneYwidth + 0.5 * zoneYwidth - (Number(String(newArr.Persistent.StrandedWorld.PlayersData["[1]Player"].Position["z"]).replace(",",".")) + (y - 2) * 630) * (height / 1000);  // local position conversion from center of map, zone is 630 high in game units
