...
class ZonePositionGenerator {
	...
	GeneratePoints(WORLD_SEED, radius, sampleRegionSize, numSamplesBeforeRejection = 30) {
		...
		let fastRandom = new FastRandom(WORLD_SEED, this.isDebug);
	...
	{
		// error calculation margin due to JS +3
		// error calculation margin due to JS -3
		if (candidate.x >= -3 && candidate.x < (sampleRegionSize.x + 3) && candidate.y >= -3 && candidate.y < (sampleRegionSize.y + 3))
		{
			...
			if (this.isDebug) {
				//console.log("IsValid : num " + num + " / num2 " + num2 + " / num3 " + num3 + " / num4 " + num4 + " / num5 " + num5 + " / num6 " + num6);
			...
			for (i = num3; i <= num4; i++)
			{
				for (j = num5; j <= num6; j++)
				{
					...
					if (num7 != -1)
					{
						...
						if (sqrMagnitude < (radius * radius) / 0.999) // error calculation margin due to JS  * 0.999
						{
							if (this.isDebug) {
