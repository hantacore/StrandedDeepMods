// Stranded deep Mapper by Hantacore - v0.22 / 0.74 big world compatibility
// specific seed issue
var debug = false;
var isDevDebug = true;
var dragging = false;
...
function writeDeveloperLog(logText)
{
	if (debugConsole2 == null)
		return;
	
	debugConsole2.value += (logText + "\n");
}
...
function updateFlags() {
	...
	if (debugConsole2 == null) 
	{
		debugConsole2 = document.getElementById("txtDebug");
	}
	debugConsole2.value = '';
	if (debug && isDevDebug)
		debugConsole2.style.visibility = "visible" ;
	else
		debugConsole2.style.visibility = "hidden" ;
...
function drawMapElements(focusOnPlayer) {
	...
	// positions now stored until reset to improve zoom performance
	if (generationZonePositions === undefined || generationZonePositions === null)
	{
		// read world seed
		...
		if (debug)
		{
			...
			writeLog("Seed : " + worldSeed);
		}

		// compute random islands positions
		// The new big world randomizer
		let myFastRandom = new FastRandom(worldSeed, debug);
		let myZonePositionGenerator = new ZonePositionGenerator(debug, isDevDebug);
		let maxIslands = 49;
		let radius = 500;
		...
		//let generatedZonePoints = myZonePositionGenerator.GeneratePointsMega(worldSeed, radius, sampleRegionSize, numSamplesBeforeRejection);
		if (debug)
		{
			var zoneindexdebug;
		...
		{
			let tempZoneIndex;
			for(tempZoneIndex = 0; tempZoneIndex < generationZonePositions.length; tempZoneIndex++)
			{
				writeDeveloperLog(generationZonePositions[tempZoneIndex].x + "$" + generationZonePositions[tempZoneIndex].y);
			}
		}
	}
	...
	for(zoneIndex = 0; zoneIndex < generationZonePositions.length; zoneIndex++)
	{
			...
			if (drawZoneNames) {
				...
				ctx.fillText("(" + zoneIndex + ") " + fullname, x - zoneXwidth + 10, y - zoneYwidth + 20);	
			...
			if (!currentZone.hasOwnProperty("Discovered") || (currentZone.hasOwnProperty("Discovered") && currentZone["Discovered"] == false)) {
				if(drawUndiscoveredIslands) {
					...
					if ((debug || drawMissions) && currentZone.Id.includes("MISSION"))
					{				
				...
				$.each(currentZone.Objects, function(name, currentvalue) {
					...
					if ((allDictionary.hasOwnProperty(key) 
						...
						&& (String(allDictionary[key]).includes("ROCK") || String(allDictionary[key]).includes("TREE") || String(allDictionary[key]).includes("PALM") || String(allDictionary[key]).includes("PLANT") || String(allDictionary[key]).includes("WRECK"))
	...
	// if ((newArr.Persistent.StrandedWorld.PlayersData.length == 1 || newArr.Persistent.StrandedWorld.PlayersData.length == 2)
...
function initDictionaries() {
	...
	allDictionary["CRAB_SPAWNER"] = "DRAW|CRAB";
	allDictionary["GIANT_CRAB_SPAWNER"] = "DRAW|CRAB";
	allDictionary["GIANT_CRAB_RAGDOLL"] = "DRAW|CRAB";
	...
	allDictionary["LIONFISH_RAGDOLL"] = "FISH";
    allDictionary["PILCHARD"] = "FISH";
	allDictionary["PILCHARD_RAGDOLL"] = "FISH";
    allDictionary["SARDINE"] = "FISH";
	allDictionary["SARDINE_RAGDOLL"] = "FISH";
    allDictionary["STING_RAY"] = "DRAW|STINGRAY";
    allDictionary["STING_RAY_RAGDOLL"] = "DRAW|STINGRAY";
	allDictionary["GROUPER"] = "FISH";
    allDictionary["GROUPER_RAGDOLL"] = "FISH";
	...
	allDictionary["SHARK - HAMMERHEAD"] = "DRAW|SHARK";
	allDictionary["SHARK - GREAT WHITE"] = "DRAW|SHARK";
	...
	allDictionary["MEDICAL_BREATH_BOOST"] = "DRAW|ITEM";
	allDictionary["MEDICAL_GAUZE"] = "DRAW|ITEM";
	allDictionary["MEDICAL_ANTIDOTE"] = "DRAW|ITEM";
	...
	allDictionary["Shelter07"] = "DRAW|SHELTER";
	allDictionary["Shelter08"] = "DRAW|SHELTER";
